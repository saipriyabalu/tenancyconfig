...
2. Update the dynamic group as follows: 
...
3. To get access to *all Generative AI resources* in your compartment and allow users to add fine-tuning training datasets to *Object Storage buckets*, use the following policy:
    ...
    allow dynamic-group genai-agent-dyn to read objects in compartment GenAI
    allow dynamic-group genai-agent-dyn to read secret-bundle in compartment GenAI 
    allow dynamic-group genai-agent-dyn to read database-tools-family in compartment GenAI
    </copy>
    ```
